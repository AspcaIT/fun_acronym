...
def create_session():
    # Options in
    return sessionmaker(bind=create_engine(sql_alchemy_connection_config("ROAR")))()
...
def pyodbc_database_connection_config(reference):
    try:
...
def sql_alchemy_connection_config(reference):
    try:
        db_config = databases_dictionary[reference]
        server, database = db_config['host'], db_config['database']
        uid, pwd = db_config['username'], db_config['password']

        connection_string = "mssql+pyodbc://%s:%s@%s:1433/%s?driver=ODBC Driver 17 for SQL Server" %\
                            (uid, pwd, server,database)
        return connection_string
    except KeyError as e:
        print("Invalid database reference supplied. The valid database keys are in the databases.yaml file\n",
              e, "\n", "Valid keys are:", databases_dictionary.keys())
